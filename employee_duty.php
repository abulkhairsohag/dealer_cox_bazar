...
<div class="right_col" role="main">
  <div class="row">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          <h2>Sales Man (Employee) List</h2>
          <div class="row float-right" align="right">
            <?php 
            if (permission_check('add_new_sales_man_button')) {
              ?>
              <a href="" class="btn btn-primary" id="add_data" data-toggle="modal" data-target="#add_update_modal"> <span class="badge"><i class="fa fa-plus"> </i></span> Add New Sales Man</a>
            <?php } ?>
          </div>
                    ...
                    <form id="form_edit_data" action="" method="POST" data-parsley-validate class="form-horizontal form-label-left">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <select required="" id="id_no" name="id_no"  class="form-control col-md-7 col-xs-12">
                            <option value=""></option>
                            <?php 
                            ...
                            if ($get_employee) {
                              while ($row = $get_employee->fetch_assoc()) {
                                $id = $row['id_no'];
                                $query = "SELECT * FROM delivery_employee WHERE id_no = '$id' and active_status = 'Active'";
                                $get_employee_delivery = $dbOb->find($query);

                                $query1 = "SELECT * FROM own_shop_employee WHERE id_no = '$id' and active_status = 'Active'";
                                $get_own_employee = $dbOb->find($query1);
                                if (!$get_employee_delivery && !$get_own_employee) {
                                  ?>
                      ...
                      <div class="form-group" style="display:none">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" min='0' step="0.01" id="per_day" name="per_day" class="form-control col-md-7 col-xs-12" >
                        </div>
                      </div>


                      <div class="form-group">
                        <label for="middle-name" class="control-label col-md-3 col-sm-3 col-xs-12">Sell Target Per Month (Taka) </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" min='0' step="0.01" id="per_month" name="per_month" class="form-control col-md-7 col-xs-12" >
                        </div>
                      </div>

                      <div class="form-group">
                        <label for="middle-name" class="control-label col-md-3 col-sm-3 col-xs-12">Extra Comission  <span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" min='0' step="0.01"  id="comission" name="comission" class="form-control col-md-7 col-xs-12" >
                        </div>
                      </div>
    ...
    $(document).on('click','.edit_data',function(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $("#per_day").val(data.per_day);
          $("#per_month").val(data.per_month);
